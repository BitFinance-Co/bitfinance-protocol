...
const BitFinance = artifacts.require('./BitFiannce.sol');
...
contract('Match', async accounts => {
        ...
        for (let i = 0; i < tokens.length; i++) {
            ...
            for (let j = 0; j < Object.keys(users).length; j++) {
                ...
                if (balancePaths && balancePaths[user] && balancePaths[user].category == 1) {
                    balance = await BitFinance.marketBalanceOf(
                        balancePaths[user].marketID,
                        token.address,
                        user
                    );
                } else {
                    balance = await BitFinance.balanceOf(token.address, user);
                }
    ...
    const matchTest = async config => {
        ...
        const res = await BitFinance.matchOrders(
            {
        ...
        logGas(res, `bitfinance.matchOrders (${makerOrders.length} Orders)`);
        ...
        if (assertFilled) {
            ...
            if (limitTaker && takerOrderParam.type == 'limit') {
                assertEqual(
                    await BitFinance.getOrderFilledAmount(takerOrder.orderHash),
                    limitTaker,
                    allowPrecisionError
                );
            }
            ...
            if (makers) {
                for (let i = 0; i < makers.length; i++) {
                    assertEqual(
                        await BitFinance.getOrderFilledAmount(makerOrders[i].orderHash),
                        makers[i],
                        allowPrecisionError
                    );
                }
    ...
    it('match with market balance with temporary liquidatable should be fine', async () => {
        const testConfig = {
            ...
            beforeMatch: async ({ baseAsset }) => {
                ...
                // assert.equal(await BitFinance.marketBalanceOf(0, baseAsset.address, u1), toWei('100'));
    ...
    it('match with a not participant relayer', async () => {
        await BitFinance.exitIncentiveSystem({ from: relayer });
    ...
    it('match with a canceled order will revert', async () => {
        const testConfig = {
            ...
            beforeMatch: async ({ takerOrder, orderAddressSet }) => {
                ...
                await BitFinance.cancelOrder(takerOrderClone, { from: takerOrderClone.trader });
            }
        };
