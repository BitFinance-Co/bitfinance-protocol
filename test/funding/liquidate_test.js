...
contract('Liquidate', accounts => {
    let marketID, BitFinance, oracle, time, ethAsset, usdAsset, res;
    ...
    it('should be a health position if there is no debt', async () => {
        let accountDetails = await BitFinance.getAccountDetails(u2, marketID);
    ...
    it('should be a unhealthy position if there are too many debts', async () => {
        ...
        res = await BitFinance.liquidateAccount(u2, marketID);
        logGas(res, 'hydro.liquidateAccount');
        // account is liquidated
        accountDetails = await BitFinance.getAccountDetails(u2, marketID);
        assert.equal(accountDetails.liquidatable, false);
    ...
    it('liquidation without debt should not result in an auction', async () => {
        ...
        // u2 has 100 usd debt
        assert.equal(await BitFinance.getAmountBorrowed(usdAsset.address, u2, marketID), toWei('100'));
        ...
        assert.equal(await BitFinance.getAuctionsCount(), '0');
        ...
        res = await mineAt(() => BitFinance.liquidateAccount(u2, marketID), time);
        logGas(res, 'hydro.liquidateAccount (no auction)');

        // u2 debt is force repaied, and no auction created
        assert.equal(await BitFinance.getAmountBorrowed(usdAsset.address, u2, marketID), '0');
        assert.equal(await BitFinance.getAuctionsCount(), '0');
    ...
    it('liquidation with debt left should result in an auction #1', async () => {
        ...
        res = await mineAt(() => BitFinance.liquidateAccount(u2, marketID), time + 86400);
        logGas(res, 'hydro.liquidateAccount (auction)');

        // u2 should have some usd debt
        assert(
            (await BitFinance.getAmountBorrowed(usdAsset.address, u2, marketID)).gt('0'),
            'debt should larger than 0'
        );

        assert.equal(await BitFinance.getAuctionsCount(), '1');
        var currentAuctionIds = await BitFinance.getCurrentAuctions();
        ...
        const auctionDetails = await BitFinance.getAuctionDetails('0');
    ...
    it('liquidation with debt left should result in an auction #2', async () => {
        ...
        // u2 has 1 eth debt
        assert.equal(await BitFinance.getAmountBorrowed(ethAsset.address, u2, marketID), toWei('1'));
        // u2 has 100 usd and 1 eth in account
        assert.equal(await BitFinance.marketBalanceOf(marketID, usdAsset.address, u2), toWei('100'));
        assert.equal(await BitFinance.marketBalanceOf(marketID, ethAsset.address, u2), toWei('1'));
        ...
        await mineAt(() => BitFinance.liquidateAccount(u2, marketID), time + 86400);
        // u2 should have some eth debt
        assert(
            (await BitFinance.getAmountBorrowed(ethAsset.address, u2, marketID)).gt('0'),
            'debt should larger than 0'
        );
        assert.equal(await BitFinance.getAuctionsCount(), '1');
        // u2 account is liquidated, status is Liqudate
    ...
    it('should return correct transferable amount #1', async () => {
        ...
        assert.equal(
            await BitFinance.getMarketTransferableAmount(marketID, usdAsset.address, u2),
            toWei('50')
        );

        assert.equal(
            await BitFinance.getMarketTransferableAmount(marketID, ethAsset.address, u2),
            toWei('0.5')
        );
    });
    ...
    const createLiquidatingAccount = async () => {
        ...
        let accountDetails = await BitFinance.getAccountDetails(u2, marketID);
        assert.equal(await BitFinance.isAccountLiquidatable(u2, marketID), true);
        assert.equal(accountDetails.liquidatable, true);
        assert.equal(accountDetails.status, CollateralAccountStatus.Normal);
        assert.equal(accountDetails.debtsTotalUSDValue, toWei('100'));
        assert.equal(accountDetails.balancesTotalUSDValue, toWei('100'));

        await mineAt(() => BitFinance.liquidateAccount(u2, marketID), time);
    ...
    it('fill healthy auction', async () => {
        ...
        await BitFinance.updateAuctionInitiatorRewardRatio(toWei('0.05'));
        await createLiquidatingAccount();

        for (let i = 0; i < 48; i++) await mine(time);

        let auctionDetails = await BitFinance.getAuctionDetails('0');
        ...
        const u1USDBalance1 = await BitFinance.balanceOf(usdAsset.address, u1);
        const u1EthBalance1 = await BitFinance.balanceOf(ethAsset.address, u1);

        const u2USDBalance1 = await BitFinance.balanceOf(usdAsset.address, u2);
        const u2EthBalance1 = web3.utils.toBN(await web3.eth.getBalance(u2));

        const initiaiorUSDBalance1 = await BitFinance.balanceOf(usdAsset.address, initiaior);
        const initiaiorEthBalance1 = await BitFinance.balanceOf(ethAsset.address, initiaior);

        /////////////////////////////////////////////////////
        // u1 has enough usd, pay 50 USD debt at ratio 50% //
        /////////////////////////////////////////////////////
        let res = await mineAt(
            () => BitFinance.fillAuctionWithAmount(0, toWei('50'), { from: u1 }),
            time
        );
        logGas(res, 'hydro.fillHealthyAuction (no truncate)');

        const u1USDBalance2 = await BitFinance.balanceOf(usdAsset.address, u1);
        const u1EthBalance2 = await BitFinance.balanceOf(ethAsset.address, u1);

        const u2USDBalance2 = await BitFinance.balanceOf(usdAsset.address, u2);
        const u2EthBalance2 = web3.utils.toBN(await web3.eth.getBalance(u2));

        const initiaiorUSDBalance2 = await BitFinance.balanceOf(usdAsset.address, initiaior);
        const initiaiorEthBalance2 = await BitFinance.balanceOf(ethAsset.address, initiaior);
        ...
        auctionDetails = await BitFinance.getAuctionDetails('0');
        ...
        res = await mineAt(() => BitFinance.fillAuctionWithAmount(0, toWei('80'), { from: u1 }), time);
        logGas(res, 'hydro.fillHealthyAuction (truncate)');

        const u1USDBalance3 = await BitFinance.balanceOf(usdAsset.address, u1);
        const u1EthBalance3 = await BitFinance.balanceOf(ethAsset.address, u1);

        const u2USDBalance3 = await BitFinance.balanceOf(usdAsset.address, u2);
        const u2EthBalance3 = web3.utils.toBN(await web3.eth.getBalance(u2));

        const initiaiorUSDBalance3 = await BitFinance.balanceOf(usdAsset.address, initiaior);
        const initiaiorEthBalance3 = await BitFinance.balanceOf(ethAsset.address, initiaior);
    ...
    it('fill bad auction', async () => {
        const initiaior = accounts[0];
        await BitFinance.updateInsuranceRatio(toWei('0.5'));
        await BitFinance.updateAuctionInitiatorRewardRatio(toWei('0.05'));
        await createLiquidatingAccount();
        ...
        assert.equal(
            (await BitFinance.getInsuranceBalance(usdAsset.address)).toString(),
            '25273972602749500'
        );
        ...
        const u2USDBalance1 = await BitFinance.balanceOf(usdAsset.address, u2);
        const u2EthBalance1 = await BitFinance.balanceOf(ethAsset.address, u2);
        ...
        const u2USDBalance2 = await BitFinance.balanceOf(usdAsset.address, u2);
        const u2EthBalance2 = await BitFinance.balanceOf(ethAsset.address, u2);
        ...
        assert.equal((await BitFinance.getInsuranceBalance(usdAsset.address)).toString(), '0');
        assert.equal(
            (await BitFinance.getTotalSupply(usdAsset.address)).toString(),
        ...
        //////////////////////////
        res = await mineAt(() => BitFinance.fillAuctionWithAmount(0, toWei('50'), { from: u1 }), time);
        logGas(res, 'hydro.fillHealthyAuction (truncate)');
    ...
    it('Auction could not be filled twice', async () => {
        await createLiquidatingAccount();
        await BitFinance.fillAuctionWithAmount(0, toWei('101'), { from: u1 }); // u1 fill the auction
        u2BorrowAmount = await BitFinance.getAmountBorrowed(usdAsset.address, u2, marketID);
        assert.equal(u2BorrowAmount.toString(), '0');

        await assert.rejects(
            BitFinance.fillAuctionWithAmount(0, toWei('101'), { from: u1 }),
