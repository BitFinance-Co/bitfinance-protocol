...
contract('LendingPool', accounts => {
    ...
    const getLendingPoolToken = async assetAddress => {
        const assetInfo = await BitFinance.getAsset(assetAddress);
    ...
    it('check interest rate', async () => {
        interestRate = await BitFinance.getInterestRates(USDAddr, 0);
        ...
        assert.equal((await BitFinance.getTotalSupply(USDAddr)).toString(), '1000616438356164383000');
        assert.equal(
            (await BitFinance.getAmountSupplied(USDAddr, u1)).toString(),
            '1000616438356164383000'
        );
        assert.equal((await BitFinance.getTotalBorrow(USDAddr)).toString(), '100616438356164383600');
        assert.equal(
            (await BitFinance.getAmountBorrowed(USDAddr, u2, 0)).toString(),
            '100616438356164383600'
        );
        indexes = await BitFinance.getIndex(USDAddr);
    ...
    it('borrow', async () => {
        ...
        logGas(res, 'borrow');
        assert.equal((await BitFinance.marketBalanceOf(0, USDAddr, u2)).toString(), toWei('200'));
        assert.equal(
            (await BitFinance.getAmountBorrowed(USDAddr, u2, 0)).toString(),
    ...
    it('repay', async () => {
        ...
        logGas(res, 'repay');
        assert.equal((await BitFinance.marketBalanceOf(0, USDAddr, u2)).toString(), toWei('50'));
    ...
    it('unsupply', async () => {
        ...
        logGas(res, 'unsupply');
        assert.equal((await BitFinance.balanceOf(USDAddr, u1)).toString(), toWei('9500'));
        poolToken = await getLendingPoolToken(USDAddr);
    ...
    it('repay all and unsupply all', async () => {
        ...
        assert.equal(
            (await BitFinance.marketBalanceOf(0, USDAddr, u2)).toString(),
            '8767123287671232800'
        );
        assert.equal((await BitFinance.getAmountBorrowed(USDAddr, u2, 0)).toString(), '0');
        res = await mineAt(
        ...
        assert.equal((await BitFinance.balanceOf(USDAddr, u1)).toString(), '10001232876712328767000'); // 100 wei remains in the system because of precision
        poolToken = await getLendingPoolToken(USDAddr);
    ...
    it('can not borrow if market closed', async () => {
        await BitFinance.setMarketBorrowUsability(0, false, { from: accounts[0] });
        await assert.rejects(
            borrow(0, USDAddr, toWei('100'), { from: u2 }),
            /MARKET_BORROW_DISABLED/
        );
        await BitFinance.setMarketBorrowUsability(0, true, { from: accounts[0] });
