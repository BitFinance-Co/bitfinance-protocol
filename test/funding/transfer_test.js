...
contract('Transfer', accounts => {
    ...
    it('multiple deposit ether unsuccessfully', async () => {
        const balanceBefore = await BitFinance.balanceOf(etherAsset, user);
        ...
        const balanceAfter = await BitFinance.balanceOf(etherAsset, user);
    ...
    it('deposit token successfully', async () => {
        ...
        const balanceBefore = await BitFinance.balanceOf(quoteAsset.address, user);
        assert.equal(balanceBefore.toString(), toWei('0'));

        // have to approve before
        await quoteAsset.approve(bitfinance.address, hugeAmount);

        res = await deposit(quoteAsset.address, toWei('1'));
        logGas(res, 'deposit');
        const balanceAfter = await BitFinance.balanceOf(quoteAsset.address, user);
    ...
    it('withdraw token successfully', async () => {
        ...
        assert.equal(await BitFinance.balanceOf(quoteAsset.address, user), toWei('1'));

        // test
        await withdraw(quoteAsset.address, toWei('1'));
        assert.equal(await BitFinance.balanceOf(quoteAsset.address, user), toWei('0'));
    });
    ...
    it('transfer ether successfully', async () => {
        ...
        const marketBalanceBefore = await BitFinance.marketBalanceOf(0, etherAsset, user);
        ...
        const marketBalanceAfter = await BitFinance.marketBalanceOf(0, etherAsset, user);
        ...
        assert.equal((await BitFinance.marketBalanceOf(1, etherAsset, user)).toString(), toWei('1'));
        assert.equal((await BitFinance.marketBalanceOf(0, etherAsset, user)).toString(), toWei('0'));
    });
    ...
    it('transfer token successfully', async () => {
        ...
        assert.equal(await BitFinance.balanceOf(quoteAsset.address, user), toWei('1'));
        assert.equal(await BitFinance.marketBalanceOf(0, quoteAsset.address, user), toWei('0'));
        ...
        assert.equal(await BitFinance.balanceOf(quoteAsset.address, user), toWei('0'));
        assert.equal(await BitFinance.marketBalanceOf(0, quoteAsset.address, user), toWei('1'));
        ...
        assert.equal(
            (await BitFinance.marketBalanceOf(1, quoteAsset.address, user)).toString(),
            toWei('1')
        );
        assert.equal(
            (await BitFinance.marketBalanceOf(0, quoteAsset.address, user)).toString(),
            toWei('0')
        );
    });
