...
contract('Batch', accounts => {
    ...
    it('deposit', async () => {
        const balanceBefore = await BitFinance.balanceOf(ethAddress, u1);
        const actions = [
            {
        ...
        const res = await BitFinance.batch(actions, { value: toWei('1') });

        logGas(res, 'deposit(batch)');

        const balanceAfter = await BitFinance.balanceOf(ethAddress, u1);
    ...
    it('withdraw', async () => {
        ...
        const res = await BitFinance.batch(actions);
    ...
    it('transfer', async () => {
        ...
        const marketBalanceBefore = await BitFinance.marketBalanceOf(0, ethAddress, u1);
        ...
        const marketBalanceAfter = await BitFinance.marketBalanceOf(0, ethAddress, u1);
    ...
    it('borrow and repay', async () => {
        ...
        res = await BitFinance.batch(u1Actions, { from: u1, value: toWei('1') });
        ...
        res = await BitFinance.batch(u2Actions, { from: u2 });
