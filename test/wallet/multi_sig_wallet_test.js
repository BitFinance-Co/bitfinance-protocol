...
contract('MultiSigWallet', accounts => {
    ...
    it('bitfinance emergency function call', async () => {
        const wallet = await newWallet();
        const BitFinance = await BitFinance.deployed();
        ...
        await BitFinance.transferOwnership(wallet.address);
        assert.equal(await BitFinance.owner(), wallet.address);

        assert.equal((await BitFinance.getMarket(0)).borrowEnable, true);

        await wallet.submitTransaction(
            BitFinance.address,
            0,
            BitFinance.contract.methods.setMarketBorrowUsability(0, false).encodeABI()
        );
        ...
        assert.equal((await BitFinance.getMarket(0)).borrowEnable, false);
    });

    it('bitfinance normal function call', async () => {
        const wallet = await newWallet();
        const BitFinance = await BitFinance.deployed();
        ...
        assert.equal((await BitFinance.getMarket(0)).withdrawRate, oldWithdrawRate);

        // updateMarket require a time lock
        await wallet.submitTransaction(
            BitFinance.address,
            0,
            BitFinance.contract.methods
                .updateMarket(
                    0,
                    toWei('0.01'),
                    toWei('0.01'),
        ...
        assert.equal((await BitFinance.getMarket(0)).withdrawRate, newWithdrawRate);
    });
